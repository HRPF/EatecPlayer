...
class myMainWindow(Ui_MainWindow, QMainWindow, CodeValidateWindow):
    ...
    def login(self):
        ...
        print("开始连接数据库")
        conn = jaydebeapi.connect('org.postgresql.Driver', url, [user, password], 'D:\Download\postgresql.jar')
        print("数据库已连接")
        curs = conn.cursor()
        print("游标已创建")
        name = self.account_text.text()
        print("已从文本框获取到用户名")
        selectsql = "SELECT * FROM eatecuser.users WHERE user_name='{}'".format(name)
